...
def get_vlnbert_models(args, config=None):
    ...
    vis_config.max_action_steps = 50  # For R2R trained model, use this. 
    # vis_config.max_action_steps = 100
    
    vis_config.imagine_enc_pano = args.imagine_enc_pano
    if(args.imagine_enc_pano):
        vis_config.max_imagination_len = args.max_imagination_len        
        vis_config.fix_imagine_embeds = args.fix_imagine_embeds
        vis_config.bypass_imag_encoder = args.bypass_imag_encoder
        vis_config.use_cosine_aux_loss = args.use_cosine_aux_loss
        vis_config.aux_loss_type = args.aux_loss_type
        vis_config.infonce_temperature = args.infonce_temperature
        vis_config.contrastive_margin_value = args.contrastive_margin_value
        vis_config.concat_imagine_with = args.concat_imagine_with
        vis_config.no_loss_test = args.no_loss_test
