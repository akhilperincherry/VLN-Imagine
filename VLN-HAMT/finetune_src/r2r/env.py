...
import torch
import torchvision.transforms as torchvis_transform
...
class R2RBatch(object):
    ...
    def __init__(
        ...
        seed=0, name=None, sel_data_idxs=None, imagine_feat_dict = None, imagine_v2_generated_flag_dict = None, sub_instr_seg_dict = None, tokenizer = None
    ):
        
        self.env = EnvBatch(connectivity_dir, feat_db=feat_db, batch_size=batch_size)
        self.img_features_db = feat_db
        self.data = instr_data
        self.scans = set([x['scan'] for x in self.data])
        # to evaluate full data
        self.gt_trajs = self._get_gt_trajs(self.data)

        self.imagine_feat_dict = imagine_feat_dict
        self.imagine_v2_generated_flag_dict = imagine_v2_generated_flag_dict
        self.sub_instr_seg_dict = sub_instr_seg_dict
        self.instr_ids_list = [data_dict['instr_id'] for data_dict in self.data] #for validating with wrong imaginations.
        self.scan_instr_id_associated = {data_dict['instr_id']:data_dict['scan'] for data_dict in self.data} #for validating with wrong imaginations further by enforcing wrong scans.

        if tokenizer is not None:
            self.tok = tokenizer
            
        if name == 'val_train_seen':
            sub_instr_seg_dict_split = 'train'
        else:
            sub_instr_seg_dict_split = name

        if name is not 'aug' and sub_instr_seg_dict_split in self.sub_instr_seg_dict.keys():
            self.instr_id_sub_instr_seg = {data_dict['instruction_id']:data_dict['instr_segmentation_indices'] for data_dict in self.sub_instr_seg_dict[sub_instr_seg_dict_split]}
            self.instr_id_nounphrase_seg = {data_dict['instruction_id']:data_dict['noun_phrase_indices'] for data_dict in self.sub_instr_seg_dict[sub_instr_seg_dict_split]}

            self.instr_id_imagine_len_dict = {key: value.count('True') for key, value in self.imagine_v2_generated_flag_dict[sub_instr_seg_dict_split].items()}
            imagine_lengths = list(self.instr_id_imagine_len_dict.values())
            imagine_len_count = Counter(imagine_lengths)
            # print(f'Split: {name}, Freq of valid imagination lengths: {imagine_len_count}')
        ...
        self.name = name #This is the dataset split name.
    ...
    def _get_nearest(self, shortest_distances, goal_id, path):
        ...
        for item in path:
            d = shortest_distances[item][goal_id] #Akhil: Finds the closest node in the taken path to the destination node.
            if d < near_d:
                near_id = item
                near_d = d
        return near_id
    ...
    def _eval_item(self, scan, path, gt_path):
        ...
        nearest_position = self._get_nearest(shortest_distances, gt_path[-1], path) #Akhil: Finds the closest node in the taken path to the destination node.

        scores['nav_error'] = shortest_distances[path[-1]][gt_path[-1]] #Akhil: How far did you end up from your goal?
        scores['oracle_error'] = shortest_distances[nearest_position][gt_path[-1]] #Akhil: How close did you get to your goal at any point in your taken path?
